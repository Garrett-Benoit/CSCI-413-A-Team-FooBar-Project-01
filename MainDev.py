...
class MainMenu(sgc.Menu): #asdf
    """ #asdf
    Create a subclass for custom functions. #asdf
    """ #asdf
    def __init__(self, **kwargs): #asdf
        super(sgc.Menu,self).__init__(**kwargs) #asdf
        self.start_new_game = False #asdf
        self.replay_1 = False #asdf
        self.replay_2 = False #asdf
        self.replay_3 = False #asdf
        
    # Return a dictionary of functions callable from the menu #asdf
    func_dict = lambda self: {"new_game": self.new_game, #asdf
                              "remove": self.remove, #asdf
                              "replay_1": self.replay_1, #asdf
                              "replay_2": self.replay_2, #asdf
                              "replay_3": self.replay_3} #asdf
                              
    # This function is called when a new game is to be started #asdf
    def new_game(self): #asdf
        self.start_new_game = True #asdf
        
    # This function is called when replay 1 is to be opened #asdf
    def replay_1(self): #asdf
        self.replay_1 = True #asdf
        
    # This function is called when replay 2 is to be opened #asdf
    def replay_2(self): #asdf
        self.replay_2 = True #asdf
    
    # This function is called when replay 3 is to be opened #asdf
    def replay_3(self): #asdf
        self.replay_3 = True #asdf


# Create menu #asdf
main_menu = None #asdf
with open("menu") as menu_file: #asdf
    main_menu = MainMenu(menu=menu_file) #asdf
...
# Main function.
def main():
    global main_menu #asdf
    
    # Initialise menu state variables #asdf
    main_menu.start_new_game = False #asdf
    main_menu.replay_1 = False #asdf
    main_menu.replay_2 = False #asdf
    main_menu.replay_3 = False #asdf
    
    # Update display #asdf
    main_menu.add() #asdf
    sgc.update(clock.tick(30)) #asdf
    pygame.display.update() #asdf
    
    # Loop for main menu processing #asdf
    while True: #asdf
        # Check for selected option #asdf
        if main_menu.start_new_game: #asdf
            break #asdf
        elif main_menu.replay_1: #asdf
            break #asdf
        elif main_menu.replay_2: #asdf
            break #asdf
        elif main_menu.replay_3: #asdf
            break #asdf
        
        # Check if window is closed #asdf
        for event in pygame.event.get(): #asdf
            sgc.event(event) #asdf
            if event.type == QUIT: #asdf
                exit() #asdf

        # Update the widgets once for each frame #asdf
        sgc.update(clock.tick(30)) #asdf
        #pygame.display.update() #asdf
        pygame.display.flip() #asdf
    
    # Update display #asdf
    time.sleep(1) #asdf
    main_menu.remove() #asdf
    input_box.add(order = 0) #asdf
    
    # Clear screen #asdf
    screen.fill((0,0,0)) #asdf
    sgc.update(clock.tick(30)) #asdf
    pygame.display.update() #asdf
    
    # process user selection #asdf
    if main_menu.start_new_game: #asdf
        new_game() #asdf
    elif main_menu.replay_1: #asdf
        open_replay(1) #asdf
...
# Function to reinitialise global variables before returning to main menu #asdf
def reinitialize(): #asdf
    global chosen_replay_filename #asdf
    global chosen_replay_file #asdf

    global maze_is_valid #asdf
    global player_made_decision #asdf
    global show_replay_1 #asdf
    global show_replay_2 #asdf
    global show_replay_3 #asdf
    global start_new_game #asdf
    global game_complete #asdf
    ...
    global door_object
    global player_object #asdf
    global chest_object_closed #asdf
    global chest_object_opened #asdf
    global key_object #asdf
    global door_object #asdf
    global simple_enemy_object #asdf
    global smart_enemy_object #asdf
    global chest_combination_1_object #asdf
    global chest_combination_2_object #asdf
    global chest_combination_3_object #asdf
    global maze_is_valid #asdf
    
    chosen_replay_filename = None #asdf
    chosen_replay_file = None #asdf

    maze_is_valid = False #asdf
    player_made_decision = False #asdf
    show_replay_1 = False #asdf
    show_replay_2 = False #asdf
    show_replay_3 = False #asdf
    start_new_game = False #asdf
    game_complete = False #asdf
    ...
    door_object = "D" #asdf
    chest_object_closed = "C" #asdf
    chest_object_opened = "O" #asdf
    key_object = "K" #asdf
    player_object = "@" #asdf
    simple_enemy_object = "E" #asdf
    smart_enemy_object = "S" #asdf
    chest_combination_1_object = str(random.randint(0, 9)) #asdf
    chest_combination_2_object = str(random.randint(0, 9)) #asdf
    chest_combination_3_object = str(random.randint(0, 9)) #asdf
